...
class CaptacionAccionesFormativas(models.Model):
    _name = 'captacion.acciones.formativas'
    ...
    accion = fields.Many2one('captacion.accionformativa', string='ACCION')
    ...
    prospector_id = fields.Many2one('captacion.prospectores', string='PARTNER')
    formador_id = fields.Many2one('captacion.formadores', string='FORMADOR')
    prospector_factura = fields.Many2one('captacion.formadores', string='FACTURACIÓN')

    recurso = fields.Selection(string='RECURSO', selection=[('interno','INTERNO'),('externo','EXTERNO')])
    estado = fields.Char('ESTADO')
    prosp_form_id = fields.Many2one('captacion.prospectores', string='FORMACION')
    
    contrata_prev = fields.Integer(string='CONTRATACIONES PREV')
    contrata_real = fields.Integer(string='CONTRATACIONES REAL')
    observaciones = fields.Char(string='OBSERVACIONES')
    docente3 = fields.Many2many('captacion.docentes', string='DOCENTES')
    participante_ids = fields.One2many('res.partner', 'acciones_formativas_id', string='PARTICIPANTE')
    participantes_ids = fields.Many2many('res.partner', relation='captacion_aformativa1_rel', string='PARTICIPANTES')
    ...
    #talleres_id = fields.Many2one('captacion.talleres')

    docaem_ids = fields.One2many('captacion.docaem', 'acciones_id', string='DOCUMENTACION')
    ...
    def _onchange_participantes_ids(self):
        ...
        if self.participantes_ids:
            ...
            return {
                ...
                'res_model': 'captacion.confirm.docaem2.wizard',  # Nombre del wizard
                'view_mode': 'form',
                'target': 'new',
                'context': {
    ...
    def create(self, vals):
        ...
        record = super(CaptacionAccionesFormativas, self).create(vals)
        return record
    ...
    def write(self, vals):
        ...
        if 'participantes_ids' in vals:
            ...
            if len(new_participantes) < len(current_participantes):
                ...
                deleted_participantes = current_participantes - self.env['res.partner'].browse(new_participantes[0][2])
                # Eliminar los registros en captacion.docaem asociados a los participantes eliminados y a esta acción formativa
                for participante in deleted_participantes:
                    # self.env['captacion.docaem'].search([('captacion_id', '=', participante.id)]).unlink()
                    self.env['captacion.docaem'].search([
                        ('acciones_id', '=', self.id),  # Filtra por la acción formativa actual
                        ('captacion_id', '=', participante.id),  # Filtra por el participante eliminado
                    ]).unlink()

                # return super(CaptacionAccionesFormativas, self).write(vals)
        
        # Llamar al método write original
        res = super(CaptacionAccionesFormativas, self).write(vals)
    ...
    def default_get(self, fields):
        res = super(CaptacionAccionesFormativas, self).default_get(fields)
        res['country_id'] = self.env.ref('base.es').id  # Pone por defecto españa
        return res

    def create_docaem_aformativa_records(self):
        """Crear registros en captacion.docaem para acciones formativas grupales."""
        for record in self:
            if record.inicio and record.fin and record.participantes_ids:
                ...
                for participante in record.participantes_ids:
                    for fecha in fechas:
                        ...
                        _logger.info(f"Registros encontrados para Acción Formativa: {len(existing_record)} para taller_id: {record.id}, captacion_id: {participante.id}, fecha: {fecha}")
                        ...
                        if not existing_record:
                            ...
                            _logger.info(f"Documento de Acción Formativa taller_id: '{record.id}' captacion_id: '{participante.id}' y fecha: '{fecha}' NO EXISTE y se crea")
                        else:
                            _logger.info(f"Documento de Acción Formativa taller_id: '{record.id}' captacion_id: '{participante.id}' y fecha: '{fecha}' EXISTE")
