...
class CaptacionContratos(models.Model):
    _name = 'captacion.contratos'
    _description = 'Registro de Contratos de captacion'


    name=fields.Char(string='CONTRATO')                                                                            #       HAY QUE HACER SECUENCIA
    empresa = fields.Char(string='EMPRESA')
    modalidad = fields.Many2one('captacion.tipo.contrato', string="REGIIMEN")
    jornada = fields.Many2one('captacion.jornada', string="JORNADA")
    pmc = fields.Char(string='PMC', related='jornada.pmc')
    tipo_contratoss = fields.Many2one('captacion.tipo.contratoss', string="TIPO CONTRATO SS")                       #      TIPO CONTRATO SEGURIDAD SOCIAL
    ...
    partner_integrador = fields.Many2one('captacion.prospectores', string='INTEGRADO POR')
    ...
    participante_id=fields.Many2one(comodel_name='res.partner', string='DNI', index=True)
    prospector_id=fields.Many2one('captacion.contratos', string='Partner')
    participante_captacion = fields.Char(related='participante_id.participante', string='PARTICIPANTE', index=True, store=True)
    provincia_captacion = fields.Char(related='participante_id.provincia', index=True, string='PROVINCIA', store=True)
    diascomputable_captacion = fields.Integer(related='participante_id.diascomputable', string="DIAS COMPUTABLES ACUM", store=True)
    documento_ids = fields.One2many('captacion.docaem','contrato_id', string='VIDA LABORAL')
    ...
    def _compute_provincia_participante(self):
        for record in self:
            #provincia = record.provincia_captacion or ""
            participante = record.participante_id.name or ""  # Usamos el campo `name` del participante
            participante_name = record.participante_captacion or ""
    ...
    def create(self, vals):
        # Crear el registro en captacion.contratos
        contrato = super(CaptacionContratos, self).create(vals)

        # Crear el registro en captacion.docaem si el campo archivo tiene contenido
        if vals.get('archivo'):
            self.env['captacion.docaem'].create({
                'captacion_id': contrato.participante_id.id,
    ...
    def write(self, vals):
        # Actualizar el registro en captacion.contratos
        res = super(CaptacionContratos, self).write(vals)

        # Si el campo archivo ha cambiado, actualizar el registro en captacion.docaem
        if 'archivo' in vals:
            for contrato in self:
                docaem = self.env['captacion.docaem'].search([('contrato_id', '=', contrato.id)], limit=1)
                if docaem:
                    docaem.write({
    ...
    def _calculate_days(self):
        for rec in self:
            ...
            if rec.fecha_fin:
                ...
                rec.diastrabajado = 1 + (rec.fecha_fin - rec.fecha_inicio ).days
            else:
                fec_ref = date.today()
                rec.diastrabajado = 1 + (fec_ref - rec.fecha_inicio ).days
            if rec.modalidad.name != 'Cuenta ajena (régimen agrario)':
                if rec.pmc=='30': # JORNADA COMPLETA
                ...
                if (fec_ref  - rec.fecha_inicio ).days > ndias:
                    rec.diascomputable = 1 + rec.porcentaje_jornada*(fec_ref - rec.fecha_inicio ).days/100
                else:
                    rec.diascomputable = 0
            
            else:
                                # ACTUALMENTE NO CONSIDERADO
                rec.diascomputable = 0
            ...
            #    peonadas_mes = self.env['captacion.contratos'].search([
           ...
           #     else:
            ...
            #else:    
            # se tabbulado una posicion menos
            #if rec.modalidad.name != 'Cuenta ajena (régimen agrario)':    
            #if rec.pmc=='30':
            ...
            #else:
            #    rec.diascomputable = 0
            # SACAR DE AQUI va a nivel usuario
