...
class CaptacionProspectores(models.Model):
    _name = 'captacion.prospectores'
    ...
    #### RELACIONES
    #document_ids = fields.One2many('captacion.documentos', 'prospector_id', string="Documentos PDF")
    docaem_ids = fields.One2many('captacion.docaem', 'captacion_id', string='Documentos')
    acciones_formativas_ids = fields.One2many('captacion.acciones.formativas', 'prospector_id', string="Acciones Formativas")
    # contratos_ids = fields.One2many('captacion.contratos', 'prospector_id', string='Contratos')
   
    contratos_ids = fields.One2many('captacion.contratos','partner_integrador', string='Contratos', order='participante_id desc' ) #'prospector_id', string='Contratos')
    ...
    def _compute_totales(self):
        for rec in self:
            ...
            participantes_unicos = rec.contratos_ids.mapped('participante_captacion')
