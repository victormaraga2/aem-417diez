...
class CaptacionImportcsvEtiquetasWizard(models.TransientModel):
    _name = 'captacion.importcsv.etiquetas.wizard'
    ...
    def process_csv(self):
        ...
        for row in reader:
            ...
            record = self.env['captacion.tipo_contratoss'].search([('name', '=', name)], limit=1)
        ...
        return {
            ...
            'res_model': 'captacion.importcsv.telefono.wizard',
            'view_mode': 'form',
            'res_id': self.id,
            'target': 'new',
        }
    ...
    def update_records(self):
        ...
        for row in reader:
            ...
            # Buscar si el registro ya existe en modelo_A
            record = self.env['res.partner'].search([('name', '=', name.upper())], limit=1)
            #record = self.env['captacion.tipo.contratoss'].search([('name', '=', name)], limit=1)
            if record:
                self.env['captacion.taghorarios'].create({
                    'name': 'JORNADA COMPLETA',
                    'sino': False,  # Representa 1 como True
                    'captacion_id': record.id
                })
                self.env['captacion.taghorarios'].create({
                    'name': 'MEDIA JORNADA',
                    'sino': False,  # Representa 1 como True
                ...
                self.env['captacion.tagprl'].create({
                    'name': '60H',
                    'sino': False,  # Representa 1 como True
                    'captacion_id': record.id
                })
                ...
                self.env['captacion.tagcolectivos'].create({
                    'name': 'MENOR 30',
                    'sino': False,  # Representa 1 como True
                    'captacion_id': record.id
                })
