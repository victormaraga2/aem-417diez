...
class CaptacionContratos(models.Model):
    ...
    def _calculate_days(self):
        for rec in self:
            ...
            if rec.diascomputable_captacion < 120:
                rec.fecha_objetivo = date.today()+timedelta(days=(((120-rec.diascomputable_captacion)*100)/(rec.porcentaje_jornada)))
            else:
                rec.fecha_objetivo = date.today()-timedelta(days=(((rec.diascomputable_captacion-120)*100)/(rec.porcentaje_jornada)))
